...
CREATE TRIGGER check_insert_ingresso_libero_null BEFORE INSERT ON `temp-test`.Persona
     FOR EACH ROW
     BEGIN
...
CREATE TRIGGER check_update_ingresso_libero_null BEFORE UPDATE ON `temp-test`.Persona
     FOR EACH ROW
     BEGIN
         IF (NEW.Nome_Piscina_P IS NULL 
            AND NEW.Data_ingresso IS NOT NULL)
...
CREATE TRIGGER check_insert_piscina_tipo_null BEFORE INSERT ON `temp-test`.Piscina
     FOR EACH ROW
     BEGIN
...
CREATE TRIGGER check_update_piscina_tipo_null BEFORE UPDATE ON `temp-test`.Piscina
     FOR EACH ROW
     BEGIN
            IF ( NEW.Tipo = 0 AND (NEW.Da IS NOT NULL OR NEW.a IS NOT NULL) )
                THEN
...
CREATE TRIGGER check_insert_partecipanti_corso BEFORE INSERT ON `temp-test`.Corso
     FOR EACH ROW
     BEGIN
            IF ( NEW.Min > NEW.Max )
                THEN
...
CREATE TRIGGER check_update_partecipanti_corso BEFORE UPDATE ON `temp-test`.Corso
     FOR EACH ROW
     BEGIN
            IF ( NEW.Min > NEW.Max )
                THEN
...
CREATE TRIGGER check_insert_ora_lezione BEFORE INSERT ON `temp-test`.Lezione
     FOR EACH ROW
     BEGIN
...
CREATE TRIGGER check_update_ora_lezione BEFORE UPDATE ON `temp-test`.Lezione
     FOR EACH ROW
     BEGIN
            IF ( NEW.Ora_dalle > NEW.Ora_alle )
                THEN
...
CREATE TRIGGER check_insert_date_rotazione BEFORE INSERT ON `temp-test`.Rotazione
     FOR EACH ROW
     BEGIN
            IF ( 
                SELECT EXISTS(
...
CREATE TRIGGER check_update_date_rotazione BEFORE UPDATE ON `temp-test`.Rotazione
     FOR EACH ROW
     BEGIN
            IF ( 
                SELECT EXISTS(
